...
class CRF(nn.Module):
    ...
    def __init__(self,
        ...
        self.device = device
        self.START_TAG_IDX, self.END_TAG_IDX = 1, 2
        self.transitions = nn.Parameter(torch.empty(num_tags, num_tags)).to(self.device)
    ...
    def _forward_alg(self, feats, mask=None):
        ...
        Args:
            feats: size=(batch_size, seq_len, self.num_tags + 2)
    ...
    def _viterbi_decode(self, feats, mask=None):
        ...
        decode_idx = torch.LongTensor(seq_len, batch_size).to(self.device)
        ...
        return path_score, decode_idx.cpu().tolist()

    def forward(self, feats, mask=None):
        if mask is not None:
            mask = mask[:, :feats.size(1)]
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        new_tags = torch.LongTensor(batch_size, seq_len).to(self.device)
